<template>
  <section
    class="border max-w-[300px] border-[#E6E6E6] font-sora h-fit rounded-2xl px-[2px] py-[16px]"
  >
        ...
        <div class="w-full rounded-lg flex flex-col gap-6 h-full">
          <div class="h-fit flex flex-col gap-2 rounded-lg">
            ...
            <div class="flex flex-col gap-2">
              <div class="w-full h-1 bg-[#F1F1F1] rounded-[40px] overflow-hidden">
                <div
                  class="h-full bg-[#3C23B5] rounded-l-[40px] transition-all duration-500 ease-in-out"
                  :style="{ width: ratingPercentage + '%' }"
                ></div>
              </div>
              <div>
                <p class="text-[#1A1A1A] text-xs font-normal">Sold: {{ sold }}/{{ total }}</p>
              </div>
            </div>
          </div>
